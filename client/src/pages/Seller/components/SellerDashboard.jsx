...
export const SellerDashboard = ({ seller }) => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/seller/checkouts/${seller._id}`);
  ...
  useEffect(() => {
    const fetchSellerRating = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/seller/sellerRating/${seller._id}`);
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/api/seller/reviews/${seller._id}`);
        if (response.status === 200) {
