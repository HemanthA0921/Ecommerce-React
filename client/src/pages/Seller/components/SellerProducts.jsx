...
export const SellerProducts = ({ seller }) => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                ...
                const response = await axios.get(`http://localhost:5000/api/seller/products/${seller._id}`);
    ...
    const deleteProduct = async (productId) => {
        try {
            const response = await axios.delete(`http://localhost:5000/api/seller/deleteProduct/${productId}`);
