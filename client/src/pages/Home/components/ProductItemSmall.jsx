...
export const ProductItemSmall = (props) => {
    ...
    const handleSaveWishlist = async () => {
        if (createNewWishlist) {
            try {
                const response = await axios.post(`http://localhost:5000/api/user/wishlists/create/${user._id}`, {
                    name: wishlistName,
                });
    ...
    const addProductToWishlist = async (wishlistId) => {
        try {
            const response = await axios.post(`http://localhost:5000/api/user/wishlists/addProduct/${wishlistId}`, {
                productId: productId,
                imagePath: imagePath,
                price: price,
